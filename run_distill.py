...
def run(output_dir="logs"):
    ...
    teacher_feature = {}
    student_feature = {}
    register_feature_hook(teacher.model, "layer2", teacher_feature) # adjust based on layer
    register_feature_hook(student, "features.0", student_feature)

    criterion_ce = torch.nn.CrossEntropyLoss()
    criterion_hint = HintLoss(student_channels=64, teacher_channels=128).to(device)
    optimizer = torch.optim.Adam(list(student.parameters()) + list(criterion_hint.parameters()), lr=1e-3)

    with PowerMonitor(outfile=f"{output_dir}/distill_power.csv") as monitor:
